            ...
            <asp:DataGrid ID="Datagrid1" runat="server" AllowPaging="True" AutoGenerateColumns="False" AllowSorting="True" CssClass="datalist" HeaderStyle-CssClass="title" GridLines="none" ItemStyle-CssClass="row">
                <Columns>
                    ...
                    <asp:TemplateColumn HeaderText="Ýade Et">
                        <ItemTemplate>
                            <a title="iade Et" id="lnkCompare" runat="server" visible='<%# IIf(items.ToString.Contains("," & CInt(Eval("Status")) & ","), "True", "False")%>' class="lightbox2 button iButton" href='<%# BaseUrl & "/store/ordercancel.aspx?IND=" & DataBinder.Eval(Container.DataItem, "OrderId") & "&MID=0"%>'>
