...
<asp:DataList ID="dlGalery" runat="server" DataKeyField="ProductId" RepeatColumns="4" RepeatDirection="Horizontal" Width="100%" EnableViewState="False">
...
<br/> <asp:HyperLink ID="lnkPub" runat="server" NavigateUrl='<%# BaseUrl & IIf(Request.QueryString.Item("tabId") = "", "/default.aspx?pub=" & ChkNullToNumber(container.dataitem("PublisherId")), "/default.aspx?tabId=" & Request.QueryString.Item("tabId") & "&pub=" & ChkNullToNumber(container.dataitem("PublisherId"))) %>' Text='<%# iif(ChkNullToNumber(container.dataitem("AuthorId"),0)>0,container.dataitem("PublisherName"),"") %>'></asp:HyperLink>
</div
<div>
