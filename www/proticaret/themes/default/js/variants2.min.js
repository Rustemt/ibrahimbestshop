function parseXml(a) {
    if ($.browser.msie) {
        var b = new ActiveXObject("Microsoft.XMLDOM"); b.loadXML(a); a = b
    }
    xmldoc = a; CreateVariants(); InitVariants(); ActiveVariants(); overVariantSize0(0); setVariantSize0(0)
}
function CreateVariants() {
    var a = 0; var b = 0; var c = 0; $(xmldoc).find("variant").each(function () {
        ...
        if ($("#size2SelectorContent button[accesskey=" + $(this).attr("size2").replace(/\//g, "-").replace(" ", "-").replace(",", "-").replace(".", "-") + "]").length == 0) {
            $("#size2SelectorContent div#sizes2").append('<button rel="variant" type="button" title="' + $(this).attr("size2") + '" id="size2_' + c + '" onmouseover="overVariantSize2(' + c + ');" onmouseout="outVariantSize2(' + c + ');" onclick="setVariantSize2(' + c + ');" class="soldOut" accesskey="' + $(this).attr("size2").replace(/\//g, "-").replace(" ", "-").replace(",", "-").replace(".", "-") + '" >' + $(this).attr("size2") + "</button>"); c = c + 1
        }
    })
}
function InitVariants() {
    $(xmldoc).find("variant[qty != '0']").each(function () { $("#size0SelectorContent input[title=" + $(this).attr("size0").replace(/\//g, "-").replace(" ", "-").replace(",", "-").replace(".", "-") + "]").attr("class", "normal"); $("#size1SelectorContent button[title=" + $(this).attr("size1").replace(/\//g, "-").replace(" ", "-").replace(",", "-").replace(".", "-") + "]").attr("class", "normal"); $("#size2SelectorContent button[title=" + $(this).attr("size2").replace(/\//g, "-").replace(" ", "-").replace(",", "-").replace(".", "-") + "]").attr("class", "normal") })
}
function ActiveVariants() {
    $(xmldoc).find("variant[qty != '0']").each(function () { if ($(this).attr("size0").replace(/\//g, "-").replace(" ", "-").replace(",", "-").replace(".", "-") != null) { Size0Active = true; $("#size0SelectorContent").show() } if ($(this).attr("size1").replace(/\//g, "-").replace(" ", "-").replace(",", "-").replace(".", "-") != null) { Size1Active = true; $("#size1SelectorContent").show() } if ($(this).attr("size2").replace(/\//g, "-").replace(" ", "-").replace(",", "-").replace(".", "-") != null) { Size2Active = true; $("#size2SelectorContent").show() } return false })
}
function CheckCartButton() {
    var a = true; if (Size0Active == true && a == true && SelectedSize0Id < 0) { a = false } if (Size1Active == true && a == true && SelectedSize1Id < 0) { a = false } if (Size2Active == true && a == true && SelectedSize2Id < 0) { a = false } if (a == true) { if ($(xmldoc).find("variant[size0 ='" + SelectedSize0 + "'][size1='" + SelectedSize1 + "'][size2='" + SelectedSize2 + "']").attr("qty") > 0) { SizesSelectedUnitId = $(xmldoc).find("variant[size0 ='" + SelectedSize0 + "'][size1='" + SelectedSize1 + "'][size2='" + SelectedSize2 + "']").attr("id"); SizesSelectedInStock = true; $(".cartbutton2").button("option", "label", AddToCartMsg) } else { SizesSelectedUnitId = $(xmldoc).find("variant[size0 ='" + SelectedSize0 + "'][size1='" + SelectedSize1 + "'][size2='" + SelectedSize2 + "']").attr("id"); SizesSelectedInStock = false; $(".cartbutton2").button("option", "label", StockOutMsg) } }
}
