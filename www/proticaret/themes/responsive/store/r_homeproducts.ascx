...
<%@ Control Language="vb" AutoEventWireup="false" Inherits="ASPNetPortal.homeproductservice"
    CodeBehind="homeproducts.ascx.vb" %>
    ...
    <div class="moduleContent">
        ...
        <asp:UpdatePanel ID="UpdatePanel1" runat="server" UpdateMode="Conditional">
            <ContentTemplate>
                                    ...
                                    <td valign="middle">
                                        <div class="homeProductFilter clearFix">
                                            <ul class="clearFix">
                                                ...
                                                <li class="chkStock">
                                                    <label class="labelInput"><asp:CheckBox ID="chkStock" runat="server" AutoPostBack="True"></asp:CheckBox> <%=GetGlobalResourceObject("lang", "ProductsInStock")%></label></li>
                                                <li>
                                                    <label>
                                                        <span>
                                                            <%=GetGlobalResourceObject("lang", "ProductOrderBy")%>
                                                            :</span>
                    ...
                    <tr>
                        <td id="pnlGalery" runat="server">
                            <div id="clickHome" class="fourContent">
                                <asp:DataList ID="dlGalery" runat="server" DataKeyField="ProductId" Width="100%"
                                    EnableViewState="False" RepeatLayout="Flow">
                                    <ItemStyle CssClass="homeProductContent radius5 moduleContentCol4" HorizontalAlign="Center"
                                    ...
                                    <ItemTemplate>
                                        <div class="productImage">
                                            <a href='<%# string.format(BaseUrl & "{0}" & ".aspx",container.dataitem("ProductUrl").tostring.Remove(container.dataitem("ProductUrl").tostring.Length - 1, 1),container.dataitem("CatId"))  %>'>
                                                <asp:Image ID="Image1q" runat="server" BorderStyle="None" AlternateText='<%# container.dataitem("ProductName") %>'
                                                    ToolTip='<%# container.dataitem("ProductName") %>' ImageUrl='<%#IIf(resizeimg=True,BaseUrl & "/uploads/productsimagesresize/" & Container.DataItem("Path"), BaseUrl & "/store/makethumb.aspx?file=" + container.dataitem("Path") & "&intSize=" & intSize) %>'>
                                                </asp:Image></a>
                                        </div>
                                        <div class="productName">
                                            <asp:HyperLink ID="lnkProductName" runat="server" NavigateUrl='<%# string.format(BaseUrl & "{0}" & ".aspx",container.dataitem("ProductUrl").tostring.Remove(container.dataitem("ProductUrl").tostring.Length - 1, 1),container.dataitem("CatId")) %>'
                                                Text='<%# container.dataitem("ProductName")  %>'></asp:HyperLink>
                                        </div>
                                        <div class="productFeatures" style="display: none;">
                                            <ul>
                                                ...
                                                <li><span>Ürün Kodu:</span>
                                                    <asp:HyperLink ID="lnkProductCode" runat="server" Text='<%# container.dataitem("ProductCode") %>'
                                                        NavigateUrl='<%# string.format(BaseUrl & "{0}" & ".aspx",container.dataitem("ProductUrl").tostring.Remove(container.dataitem("ProductUrl").tostring.Length - 1, 1),container.dataitem("CatId")) %>'></asp:HyperLink>
                                                </li>
                                                <li><span class="description"></span>
                                                    <asp:Label ID="Label5q" runat="server" Text='<%# container.dataitem("Description") %>'>Açýklama:</asp:Label>
                                                </li>
                                            </ul>
                                        </div>
                                        ...
                                        <div id="ud_container">
                                            <a id="ud" href='<%# BaseUrl & container.dataitem("Url")  %>' style='display: <%# iif(container.dataitem("ProductId") = container.dataitem("prodid"),"","none") %>'
                                                rel='lightbox' class='lightbox'></a>
                                        </div>
                ...
                <asp:UpdateProgress ID="mup" runat="server" DisplayAfter="0" BackgroundCssClass="modalBackground">
                    <ProgressTemplate>
                        <div class="modalPopup">
                            <p>
                                <%=GetGlobalResourceObject("lang", "Loading")%><img alt="Yükleniyor" src="<% =getThemePath() %>/images/loading.gif"
                                    align="middle" /></p>
                            <p>
                                <%=GetGlobalResourceObject("lang", "PleaseWait")%></p>
                        </div>
                    </ProgressTemplate>
                </asp:UpdateProgress>
            </ContentTemplate>
