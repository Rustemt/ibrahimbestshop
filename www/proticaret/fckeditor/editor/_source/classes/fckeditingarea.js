...
FCKEditingArea.prototype.Start = function( html, secondCall )
{
	...
	if ( this.Mode == FCK_EDITMODE_WYSIWYG )
	{
		...
		{
			window._FCKHtmlToLoad = html.replace( /<head id="head1" runat="server">/i, '<head id="head1" runat="server">' + sOverrideError ) ;
		...
		if ( !FCK_IS_CUSTOM_DOMAIN || !FCKBrowserInfo.IsIE )
		{
			...
			oDoc.open() ;
			oDoc.write( html.replace( /<head id="head1" runat="server">/i, '<head id="head1" runat="server">' + sOverrideError ) ) ;
			oDoc.close() ;
		}
