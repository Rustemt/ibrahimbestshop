<%@ Control Language="vb" AutoEventWireup="false" CodeBehind="iparabanklist.ascx.vb" Inherits="ASPNetPortal.iparabanklist" %>
<div class="pageIParaBankList">
        ...
        <asp:DataGrid ID="Datagrid1" runat="server" GridLines="None" AutoGenerateColumns="False" HeaderStyle-CssClass="title" FooterStyle-HorizontalAlign="Center">
			<Columns>
                ...
                <asp:TemplateColumn HeaderText="<%$ Resources:admin,BankName %>">
                    <ItemTemplate>
                        <asp:HyperLink ID="HyperLink3" runat="Server" CssClass="btnLink fancyContent" NavigateUrl='<%# "/admin/ecommerce/paytypes/ipara/editiparabank.aspx?IND=" & DataBinder.Eval(Container.DataItem, "BankId") & "&MID=" & 0 & ""%> '><%#Eval("BankName")%></asp:HyperLink>
                    </ItemTemplate>
                </asp:TemplateColumn>
                ...
                <asp:TemplateColumn HeaderStyle-HorizontalAlign="Center" Visible="false" ItemStyle-HorizontalAlign="Center" HeaderText="Düzenle">
                    <ItemTemplate>
                        <asp:HyperLink ID="HyperLink1" runat="Server" CssClass="btnIcon fancyContent" NavigateUrl='<%# "/admin/ecommerce/paytypes/ipara/editiparabank.aspx?IND=" & DataBinder.Eval(Container.DataItem, "BankId") & "&MID=" & 0 & ""%> '><i class="fa fa-cog"></i> <%= GetGlobalResourceObject("admin","Edit") %></asp:HyperLink>
                    </ItemTemplate>
                </asp:TemplateColumn>
