<%@ Control Language="vb" AutoEventWireup="false" CodeBehind="kvkpos.ascx.vb" Inherits="ASPNetPortal.kvkpos" %>
<div class="pageKVKPos">
        ...
        <asp:DataGrid ID="Datagrid1" runat="server" GridLines="None" AutoGenerateColumns="False" HeaderStyle-CssClass="title" FooterStyle-HorizontalAlign="Center">
            <Columns>
                ...
                <asp:TemplateColumn HeaderText="<%$ Resources:admin,BankName %>">
                    <ItemTemplate>
                        <asp:HyperLink ID="HyperLink3" runat="Server" CssClass="btnLink fancyContent"  NavigateUrl='<%# "/admin/ecommerce/paytypes/kvkpos/editkvkbank.aspx?IND=" & DataBinder.Eval(Container.DataItem, "BankId") & "&MID=" & 0 & ""%> '><%#Eval("BankName")%></asp:HyperLink>
                    </ItemTemplate>
                </asp:TemplateColumn>
                ...
                <asp:TemplateColumn Visible="false" HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center" HeaderText="Düzenle">
                    <ItemTemplate>
                        <asp:HyperLink ID="HyperLink1" runat="Server" CssClass="btnIcon fancyContent" NavigateUrl='<%# "/admin/ecommerce/paytypes/kvkpos/editkvkbank.aspx?IND=" & DataBinder.Eval(Container.DataItem, "BankId") & "&MID=" & 0 & ""%> '><i class="fa fa-cog"></i> <%= GetGlobalResourceObject("admin","Edit") %></asp:HyperLink>
                    </ItemTemplate>
                </asp:TemplateColumn>
