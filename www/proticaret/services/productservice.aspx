...
<script language="VB" runat="server">  	
    ...
    Public Shared Function GetCargoTotal() As String
        Dim ret As String = "0.00 TL"
            
            
             
        Dim tmpOrder As TempOrder = HttpContext.Current.Session("TempOrder")
    ...
    <WebMethod()> _
    Public Shared Function GetProducts(ByVal CatId1 As String, ByVal CatId2 As String, ByVal CatId3 As String, OrderBy As String, OnlyStock As String, ByVal pageIndex As Integer) As String
        If CatId1 <> "" Then

            If OrderBy <> "" Then
                HttpContext.Current.Response.Cookies("OrderBy").Value = OrderBy
            Else
                OrderBy = "OrderByDefault"
                HttpContext.Current.Response.Cookies("OrderBy").Value = "OrderByDefault"
            End If

            If OnlyStock <> "" Then
                HttpContext.Current.Response.Cookies("StockFilter").Value = OnlyStock
            Else
                OnlyStock = "False"
                HttpContext.Current.Response.Cookies("StockFilter").Value = "False"
            End If
        ...
        "SellNoneStock,Code8,Code9,Code10,Code11,Code12,Code13,Code14,Code15,HizliKargo,EnUcuzUrun,AyniGunTeslim,OzelUrun,IndirimliUrun,SinirliSayidaUrun,FirsatUrunu,YeniUrun,SokFiyatliUrun,HediyeliUrun from ProductList  " & tekstilsql & "  WHERE Lineage like '%' + @CatId + '%' and PortalId = @PortalId "
            ...
            da.Fill(dx)
            Dim RowCount As Integer = dx.Tables(0).Rows.Count

            Dim RecordsPerPage As Integer = ConfigurationManager.AppSettings("ProductsPerPage")
            da.Fill(ds, (pageIndex - 1) * RecordsPerPage, RecordsPerPage, "Products")

            Dim dt As DataTable = New DataTable("PageCount")
            dt.Columns.Add("PageCount")
            dt.Rows.Add()
            dt.Rows(0)(0) = RowCount \ RecordsPerPage
            ds.Tables.Add(dt)
