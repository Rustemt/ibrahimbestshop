<div class="panel panel-default">
    ...
    <div class="panel-body pan">
        <form action="/admin/urun/ajaxBul" class="form-horizontal vertical-offset-2" method="post">
            <div class="form-body">
            ...
            <div class="form-actions pal">
                <button type="button" class="btn btn-primary btn-outlined" onclick="islem.guncelle()">Güncelle </button>
                &nbsp;
                <button type="button" class="btn btn-green btn-outlined" onclick="islem.sil()">Sil</button>
            </div>
        </form>
    </div>
</div>
...
var islem = {
		...
		urunBul : function(urunInput){
			$.post('/admin/urun/ajaxBul', {barkod:urunInput}, function(cevap){
      ...
      $('#urunFoto').val(cevap.urunFoto);
		...
		sil : function(urunSil){
			$.post('/admin/urun/ajaxSil', {id:islem.id}, function(cevap){
				console.log(cevap);
			});
		},
		guncelle : function(urunGuncelle){
			$.post('/admin/urun/ajaxGuncelle',
					{id:islem.id,title:$('#urunBaslik').val(),
          urunFiyat:$('#urunFiyat').val(),
          urunAciklama:$('#urunAciklama').val(),
          urunAd:$('#urunAd').val(),
          urunFoto:$('#urunFoto').val()},
					function(cevap){
				console.log(cevap);
			});
		}
